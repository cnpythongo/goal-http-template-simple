...
func GetHistoryById(db *gorm.DB, id int64) (*History, error) {
	result := NewHistory()
...
func DeleteHistoryByIds(db *gorm.DB, ids []int64) error {
	return db.Model(NewHistory()).Where("id in ?", ids).Update("delete_time", time.Now().Unix()).Error
}

func DeleteHistoryByUserIds(db *gorm.DB, userIds []int64) error {
