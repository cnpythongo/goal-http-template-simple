...
func Init() *redis.Client {
	...
	opt, err := redis.ParseURL(fmt.Sprintf("redis://%s:%d", cfg.Host, cfg.Port))
	if err != nil {
		log.GetLogger().Fatal("initRedis redis.ParseURL err: ", err)
		panic(err)
	}
	opt.Password = cfg.Auth
	opt.MaxIdleConns = cfg.MaxIdle
	opt.MaxActiveConns = cfg.MaxActive
	opt.PoolSize = 100
	...
	if err != nil {
		log.GetLogger().Fatal("initRedis client.Ping err: ", err)
		panic(err)
	}

	RedisUtil = &redisUtil{redis: redisClient}
	return redisClient
}
...
func (ru redisUtil) Info(sections ...string) (res map[string]string) {
	...
	if err != nil {
		log.GetLogger().Errorf("redisUtil.Info err: err=[%+v]", err)
		return res
	}
	// string拆分多行
	lines, err := stringToLines(infoStr)
	if err != nil {
		log.GetLogger().Errorf("stringToLines err: err=[%+v]", err)
		return res
	}
	// 解析成Map
...
func (ru redisUtil) DBSize() int64 {
	...
	if err != nil {
		log.GetLogger().Errorf("redisUtil.DBSize err: err=[%+v]", err)
		return 0
	}
	return size
}
...
func (ru redisUtil) Set(key string, value interface{}, timeSec int) bool {
	...
	if err != nil {
		log.GetLogger().Errorf("redisUtil.Set err: err=[%+v]", err)
		return false
	}
	return true
}
...
func (ru redisUtil) Get(key string) string {
	res, err := ru.redis.Get(context.Background(), RedisPrefix+key).Result()
	if err != nil {
		log.GetLogger().Errorf("redisUtil.Get err: err=[%+v]", err)
		return ""
	}
	return res
}
...
func (ru redisUtil) SSet(key string, values ...interface{}) bool {
	err := ru.redis.SAdd(context.Background(), RedisPrefix+key, values...).Err()
	if err != nil {
		log.GetLogger().Errorf("redisUtil.SSet err: err=[%+v]", err)
		return false
	}
	return true
}
...
func (ru redisUtil) SGet(key string) []string {
	res, err := ru.redis.SMembers(context.Background(), RedisPrefix+key).Result()
	if err != nil {
		log.GetLogger().Errorf("redisUtil.SGet err: err=[%+v]", err)
		return []string{}
	}
	return res
}
...
func (ru redisUtil) HMSet(key string, mapping map[string]string, timeSec int) bool {
	err := ru.redis.HSet(context.Background(), RedisPrefix+key, mapping).Err()
	if err != nil {
		log.GetLogger().Errorf("redisUtil.HMSet err: err=[%+v]", err)
		return false
	}
	if timeSec > 0 {
...
func (ru redisUtil) HGet(key string, field string) string {
	res, err := ru.redis.HGet(context.Background(), RedisPrefix+key, field).Result()
	if err != nil {
		log.GetLogger().Errorf("redisUtil.HGet err: err=[%+v]", err)
		return ""
	}
	return res
}
...
func (ru redisUtil) HExists(key string, field string) bool {
	res, err := ru.redis.HExists(context.Background(), RedisPrefix+key, field).Result()
	if err != nil {
		log.GetLogger().Errorf("redisUtil.HExists err: err=[%+v]", err)
		return false
	}
	return res
}
...
func (ru redisUtil) HDel(key string, fields ...string) bool {
	err := ru.redis.HDel(context.Background(), RedisPrefix+key, fields...).Err()
	if err != nil {
		log.GetLogger().Errorf("redisUtil.HDel err: err=[%+v]", err)
		return false
	}
	return true
}
...
func (ru redisUtil) Exists(keys ...string) int64 {
	...
	if err != nil {
		log.GetLogger().Errorf("redisUtil.Exists err: err=[%+v]", err)
		return -1
	}
	return cnt
}
...
func (ru redisUtil) Expire(key string, timeSec int) bool {
	err := ru.redis.Expire(context.Background(), RedisPrefix+key, time.Duration(timeSec)*time.Second).Err()
	if err != nil {
		log.GetLogger().Errorf("redisUtil.Expire err: err=[%+v]", err)
		return false
	}
	return true
}
...
func (ru redisUtil) TTL(key string) int {
	td, err := ru.redis.TTL(context.Background(), RedisPrefix+key).Result()
	if err != nil {
		log.GetLogger().Errorf("redisUtil.TTL err: err=[%+v]", err)
		return 0
	}
...
func (ru redisUtil) Del(keys ...string) bool {
	...
	if err != nil {
		log.GetLogger().Errorf("redisUtil.Del err: err=[%+v]", err)
		return false
	}
	return true
}
